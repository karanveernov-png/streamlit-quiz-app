...
def quiz_reset():
    for k in ["question_index", "score", "wrong_answers", "start_time", "quiz_radio"]:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Placeholder for timer-based skips so it renders cleanly
    timer_skip_placeholder = st.empty()

    if remaining <= 0:
        ca = q["answer"]
        timer_skip_placeholder.error("⏰ Time is up! Moving to the next question...")
        ...
        st.session_state.start_time      = time.time()
        st.session_state["quiz_radio"]   = None
        st.rerun()
    ...
    with st.form(key="quiz_form", clear_on_submit=False):
        ...
        # THIS IS THE MAGIC FIX: A dedicated placeholder inside the form.
        # It guarantees old messages are wiped out instantly.
        ...
        with bc:
            ...
            if submit_btn:
                if selected is None:
                    # Renders into the placeholder instead of appending to the form
                    feedback_placeholder.warning("⚠️ Please select an option first.")
                    time.sleep(1.5)
                    st.rerun()
                else:
                    if selected == q["answer"]:
                        # Renders into the placeholder instead of appending
                        feedback_placeholder.success("✅ Correct! Great job!")
                        st.session_state.score += 1
                    else:
                        ca = q["answer"]
                        # Renders into the placeholder instead of appending
                        feedback_placeholder.error(f"❌ Wrong! Correct: **{ca} → {q['options'][ca]}**")
                        st.session_state.wrong_answers.append({
                            "question":    q["question"],
