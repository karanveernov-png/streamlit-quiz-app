...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Handle auto-skip before rendering the question to avoid ghost UI
    if remaining <= 0:
        ca = q["answer"]
        ...
        st.session_state.start_time      = time.time()
        # Manually clear the constant radio widget state
    ...
    # ── FORM: Answer Options (Constant Keys prevent DOM crossfade bugs) ──
        ...
        with bc:
            ...
            if submit_btn:
                if selected is None:
                    ...
                    st.session_state.question_index += 1
                    st.session_state.start_time = time.time()
                    # Manually clear the radio state for the next question
                    st.session_state["quiz_radio"] = None
                    st.rerun()
