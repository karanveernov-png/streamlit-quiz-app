...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if remaining <= 0:
        ...
        time.sleep(1.5) # Give the user a moment to see the timeout message
    ...
    # ── FORM: Answer Options (Prevents premature reruns) ──
    with st.form(key=f"form_q_{idx}", clear_on_submit=True):
        selected = st.radio(
            "Your answer",
            list(q["options"].keys()),
            format_func=lambda k: f"  {k}  ·  {q['options'][k]}",
            index=None,
            key=f"radio_q_{idx}" # Unique key per question
        )
